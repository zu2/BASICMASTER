...
モニタには少なくともV1.1(MB6880)とV1.2(MB6881/5) がある。V1.0は私は未見

- マニュアル記載の「モニターのサブルーチン」は共通して使える。
- V1.2 では$FFE3にアドレス入力(FROM/TO）があるが、これはV1.1では使えない。
- アドレス入力(FROM/TO）$F682 は共通して使える。
- ファイル名入力 $F4CF も共通して使える。
...
回路・動作については、下記資料にある日立H68/TRの「デバッグ用ステップラン機構」と同様である。
H68/TRでは10マシンサイクル後にNMIをかけるようになっていて1サイクル異なっている。

- [トレーニングモジュールの機能設定 / 日立システム開発研究所 / 情報学広場：情報処理学会電子図書館](https://ipsj.ixsq.nii.ac.jp/records/25277)
