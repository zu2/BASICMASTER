...
バイナリフォーマット $01 はテープに記録されたアドレス（SAVE時に指定したアドレス）に読み込まれ、そのアドレスがLDTOP($003B,3C)に書かれる。複数ブロックがある場合も、テープに記録されたアドレスに読み込む。

テキストフォーマット $10 はLDTOP($003B,3C)で指定したアドレスに読み込む。複数ブロックがあるときは、順次 +256されたアドレスに読み込む。

モニタのBYTOUTサブルーチンでは、拡張子がBのときはバイナリ($01)、Bのときはテキスト($10)としてSAVEされる。


|用途				|バイト数|詳細|ワークエリアアドレス|
|-------------------|-------|-------------------------------------------|-----------------------|
|プリアンブル		|65		|$FF の連続||
|開始マーク			|1		|$01||
|ブロック種別		|1		|$00:最終ブロック、$00:最終 $01:固定 $10:可変|$004B|
|ブロック番号A		|1		|BASICのブロック番号|$004C|
|ファイル名			|6		|6文字に満たない場合は、後ろに空白を詰める|$004D-($0043-のコピー）|
|デリミタ			|1		|'.' ($2E)|$0053($0049)|
|フォーマット		|1		|'B' または 'S' ($42 or $53)|$0054($004A)|
|ブロック番号B		|1		|複数ブロックが連続するときの番号|$0055|
|ブロックサイズ		|1		|1-256. $00のときは256と見なす|$0056|
|アドレス			|2		|上位・下位の順|$0057-$0058($0037-$0038)|
...
LOAD時にはブロック番号B（常に01）と、ブロック番号A（BASIC側の番号）が表示される。ブロック番号Aの表示はBASICインタプリタ側で行なっている。

BASICの最終ブロックは2バイトだけで、$0600-$0601に$FFFFが書かれる。LOAD時は、ここに$FFFFが書かれていれば最終ブロックであると認識する。
...
BYTOUTは1回だけ呼ばれる。LOAD時に表示されるのはブロック番号B。
...
日立エディタ・アセンブラでSAVEしたソースは、大きなブロックが256バイト以下に分割されて連なる形式。
BYTOUTは1回だけ呼ばれる。
（私が移植したKUMAJIRIのRANPAKUエディタも同様）

LOAD時は01ブロックは BYTIN呼び出し時に$0037-$0038で指定したアドレスに読み込まれる。アドレスを256バイトずつ増やして読み込みを行う。テープに記録されているアドレスは使わない。
...
|---------------------------|-------|---------------------------------------|
|bit 1の連続				|256||
|$003B-$004Cの内容をベタ書き|18|詳細は下記|
|   003B,003C               |2|先頭アドレス|
|   003D,003E               |2|終了アドレス|
|   003F,0040               |2|（ダミー:COPYTOP）|
|   0041,0042               |2|（ダミー:COPYEND）|
|   0043-0048               |6|ファイル名。6文字に足りない部分は後ろに空白|
|   0049                    |1|'.'|
|   004A                    |1|'B' または 'S'|
|   004B                    |1|ブロック種別,$00:最終,$01:固定,$10:可変|
|   004C                    |1|ブロック番号|
|($3B,$3C)-($3D,$3F)		|n|データ内容|
|bit 1の連続				|48||
...
ブロック種別〜FNAME2 は300bpsテープ上の並びと同じ
...
|ブロック種別			|1		|$004B $00:最終 $01:固定 $10:可変|
