//
//	Convert UNICODE text to BASICMASTER FULLKEY text
//
//			by ZUKERAN, shin
//		(@zu2, aka TEW SOFT)
//
//	FULLKEY: see. I/O 1980.3 P.89
//                I/O 1989.8 P.96
//
#include	<stdio.h>
#include	<stdlib.h>
...
//	fprintf(stderr,"offset:%04x\n",offset);
	...
	long filesize;
	if (argc<2 || strcmp(filename,"-")==0){				// from stdin
		const long filemax = 0x10000;			// may be <64KB
		if((buffer = (unsigned char *)malloc(filemax+1))==NULL){
				perror("can't malloc");
				exit(1);
		}
		filesize = 0;
		...
		while((ch=fgetc(stdin))!=EOF){
			buffer[filesize++]=ch;
			if(filesize>=filemax){
				fprintf(stderr,"stdin too large %ld>=%ld",filesize,filemax);
				exit(1);
			}
		}
