...
## ベーシックマスターに移植されたGAME言語について

ASCII誌で公開されたGAME言語（H68/TR版GAME III）は、H68/TRに合わせたメモリマップになっている。H68/TRは最大RAM 3KB($0000-$0BFF）しかない。

- ゼロページを大量に使う（入力バッファやスタックも）
- インタプリタは$0100-$06FD
- I/Oルーチン等の領域が$700-$7FF
- プログラム格納域は$0800-

ベーシックマスターは$0100-$03FFにVRAMがあり、$400-もBASICインタプリタが使用している。MB-6880は標準8KBしかメモリがないので、初期に移植されたGAMEインタプリタはBASICのワークも潰して実装されている。

### GAME on MB-6880    V.2

- インタプリタは$0600-$08FF,$0A00-$0FFF
	- $0900-$09FF はインタプリタ入力で壊れるので避けている
	- $0A00-$0FFF は拡張部分
- TL/1-BM のエディタとしても使われた

### GAME-MB Ver 2.0

- インタプリタは$1000-$19AE
	- MUSIC, CURSORなどが拡張されている
